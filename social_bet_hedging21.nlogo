...
to setup
  ...
  [
    ...
    set age (random (365 * 7)) + (365 * 2) ; Age of the bat, in days (time steps)
...
to feed
  ...
  [
    ...
    set p_succ foraging / (1 + exp(-0.005 * (age - 300))) ; Determine the probability of a successful hunt
    ifelse b < p_succ
...
to move
  ...
  [ ; If older than two years old
    ...
    let bat-count-here count turtles with [roost = [roost] of myself and age > 300]
    ;show bat-count-here
    ;show roost-switch-prob
    ...
    [ ; If there is a roost switch
      ...
      [ ; If there is no social preference
        ask patches with [roost < 13]
        [
          let a roost
          set bat-count count turtles with [roost = a and age > 300]
        ]
      ...
      [ ; If there is social preference
        ...
        [ ; While we haven't found a roost
          ...
          [ ; If there are any bats present in the checked roost
            let bat-count-local count turtles with [roost = random-roost and age > 300] ; Count the number of bats
            ;show bat-count-local
...
to groom
  ...
  [ ; If there are any other bats
    ...
    foreach sorted-values [val ->
      let idx (first filter [z -> item z valueList = val and not member? z used-indices] n-values length valueList [z -> z])
      if idx != nobody [
        set sorted-who-list lput (item idx whoList) sorted-who-list
        set used-indices lput idx used-indices
      ]
    ]
    ;show sorted-values
    if decision = 0
  ...
  [ ; If the bat didn't find food
    ...
    [ ; And if there are any
      ...
      [ ; While we haven't looked at every bat
        ...
        let b random-float 100 ; Generate a random number
        ...
        [ ; Ash the potential donating bat
          set R donate_give;min (list 5 donate_give) ; To set the reserves to how much they can donate
        ...
        [
          ...
          [ ; If marked to donate, and there are reserves to share
            ...
            [ ; While the bat is not done feeding and the bat is not fed
              set b random-float 100 ; Set a new random number
...
to go
  ...
  if ((count turtles with [age > 300] >= (12 * limit)) and (pop-bust = FALSE))
  [
    set pop-bust TRUE
    show "pop-bust"
  ]
