...
to feed
  ...
  set p_succ 0.75 / (1 + exp(-0.01 * (age - 500))) ; Determine the probability of a successful hunt
  ifelse b < p_succ
  ...
  [ ; If older than two years old
    ...
    [ ; If there is a roost switch
      ...
      [ ; If there is social preference
        ...
        if relation-sum != 0
        [
          set props map [x -> x / relation-sum] row1 ; Find the percent of the total score for each bat
        ]
        ifelse relation-sum = 0
        [ ; If the bat has no friendly relationships
          move-to one-of patches with ([roost != [roost] of myself and bat-count <= limit]) ; Move to a random different roost
        ]
        [ ; Otherwise
          ...
          [ ; And while we haven't looked at each roost
            ...
            [ ; If we aren't looking at the same roost
              ...
              [ ; If there are any turtles in the focal roost
                let bat-count-local count turtles with [roost = (i + 1)]
                ifelse bat-count-local <= limit
                [
