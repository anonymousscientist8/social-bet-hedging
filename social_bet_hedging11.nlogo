...
to setup
  ...
  [
    ...
    set relation n-values Bats [25] ; Vector relationship with all other bats in the system (100,000 is arbitrary)
    set relation replace-item who relation 0
...
to feed
  ...
  [ ; If the bat found food
    ...
    ifelse cheat = 1
    [
      set donate_give need
    ]
    [
      set donate_give need + extra ; Reset the amount of food the bat can donate to 5 units
    ]
  ...
  [ ; Otherwise, if the bat did not find food
    ...
    set donate_need need ; Reset the amount of food the bat needs for donation to 3 units
  ]
    ...
    [ ; If there is a roost switch
      ...
      [ ; If there is social preference
        ...
        [ ; If we haven't found a suitable place to go
          move-to one-of patches with ([roost != [roost] of myself and bat-count <= limit]) ; Move to a random roost
        ]
        [ ; If we have
...
to move-children
  ...
  [ ; If less than two years old
    ...
    [
      ifelse (age < (7 * 30))
      [ ; If less than seven months of age, the bat will not be able to survive
        let dead-who who ; Find the id of the bat dying
        let index position dead-who relation_id ; Find the position in the list
        set relation_id remove dead-who relation_id ; Remove from relation list
        ask turtles with [who != dead-who]
...
to groom
  ...
  [ ; If there are any other bats
    ...
    [ ; If diversifyinig strategy
      set i 0 ; Create an index
      let bat-list [] ; Initialize a list to store bat already groomed
      let divisors 20 ; The number of bats groomed
      let boost groom_give / divisors ; The boost from grooming each bat
      while [i < divisors]
      [ ; While there are still bats to groom
        ifelse (divisors < (length flattened-who-list))
    ...
    [ ; And if there are any
      ...
      [ ; While we haven't looked at every bat
        ...
        [ ; If marked to donate, and there are reserves to share
          ...
          set days-since-eat days-since-eat - 1 / 3
          ...
          set relation replace-item index1 relation (relation-score + 0.5) ; Improve relationship
          ...
          [ ; ask the donating bat
            ...
            if donate_give < extra
            [
              set days-since-eat days-since-eat + 1 / need
            ]
        ...
        [ ; If refused to donate
          ...
          set relation replace-item index2 relation (relation-score - 0.5) ; Degrade relationship
...
to starve
  let p_starve 1 / exp(-5 * days-since-eat + 15)
...
to go
  ...
  ask turtles
  [ ; Turtles find food
    set groom_give 1 ; Maximum percentage increase from grooming
  ...
  [ ; If we have gone through Days ticks, stop the program
    ...
    show "magenta"
    show count turtles with [color = magenta]
    show "pink"
