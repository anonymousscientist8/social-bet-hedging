...
to setup
  ...
  set p_die 0.001 ; probability of being killed by a predator
  ...
  [
    ...
    [
      set color black
      set decision 1; Determines whether a bat's propensity to seek new (0) or familiar individuals (1).
                    ; A decision value of 2 will be implemented for when bats adjust their strategy in life
    ]
    [
      set color green
...
to feed
  ...
  set p_succ 0.9 / (1 + exp(-0.01 * (age - 500))) ; Determine the probability of a successful hunt
  ifelse b < p_succ
  ...
  [ ; If older than two years old
    ...
    [ ; If there is a roost switch
      ...
      [ ; If there is no social preference
        move-to one-of patches with ([roost != [roost] of myself]) ; Move to a different patch
      ]
      [ ; If there is social preference
        let row1 matrix:get-row relation 0 ; Get the relationship matrix
        let relation-sum sum row1 ; Find the total relationship with all other bats
        let props map [x -> x / relation-sum] row1 ; Find the percent of the total score for each bat
        ifelse relation-sum = 0
        [ ; If the bat has no friendly relationships
          move-to one-of patches with ([roost != [roost] of myself]) ; Move to a random different roost
        ]
        [ ; Otherwise
          let i 0 ; Create an index
          let roost-props n-values Roosts [0] ; Create Roosts length vector of zeros
          while [i < Roosts]
          [ ; And while we haven't looked at each roost
            ifelse (i + 1) != roost
            [ ; If we aren't looking at the same roost
              ifelse (any? turtles with [roost = (i + 1)])
              [ ; If there are any turtles in the focal roost
                ...
                while [j < (length matching-bats)]
                [ ; And while we have't looked at every matching bat
                  let current-bat item j matching-bats ; Isolate the bat
                  let current-prop item current-bat props ; Find the focal bat's relationship
                  let past-prop item i roost-props ; Store the last probability to go to this roost
                  set roost-props replace-item i roost-props (past-prop + current-prop) ; Update the probability based on the presence of this bat
                  set j j + 1 ; And update the index
                ]
                set i i + 1 ; And update the index
              ]
              [ ; If there's not other bats in this roost
                set i i + 1 ; We don't need to update roost-props
              ]
            ]
            [ ; If we are loking at the same roost
              set i i + 1 ; We don't need to update roost-props
            ]
          ]
          let props-sum sum roost-props ; Now find the total of the proportions
          ifelse (props-sum != 0)
          [ ; And if the total is non-zero (there are bats in other roosts)
            set roost-props map [x -> x / props-sum] roost-props ; Get the true probability of going to each roost
            let c random-float 1 ; Generate a random number
            let k 0 ; And an index
            let found? False ; And a way of determining whether a new roost has been found
            while [(k < (length roost-props)) and (found? = False)]
            [ ; While we haven't looked at each roost and we haven't found a place to go
              let roost-sum 0 ; Create a variable roost-sum
              ifelse k = 0
              [ ; And if this is the first roost
                set roost-sum item k roost-props ; Just store the first roost variable
              ]
              [ ; Otherwise
                let roost-sublist sublist roost-props 0 (k + 1) ; Make a sublist
                set roost-sum sum roost-sublist ; And find the sum of that sublist
              ]
              if c < roost-sum
              [ ; Then if we found a place to go
                move-to one-of patches with [roost = k + 1] ; Move to the right patch
                set found? True ; Make it as such
              ]
              set k k + 1 ; And update the index
            ]
          ]
          [ ; And if there aren't bats in other roosts
...
to move-children
  ...
  if (age < (30 * 7)) and (any? turtles with [who = [mother] of myself] = False)
  [ ; If less than 7 months old and the mother while foraging, the bat dies too
    die
  ]
end
...
to groom
  ...
  [ ; If there are any other bats
    ...
    [ ; If diversifyinig strategy
      let i 0 ; Create an index
      let bat-list [] ; Initialize a ist to store bat already groomed
      let divisors 5 ; The number of bats groomed
    ...
    [ ; If focusing strategy
      ...
      let boost1 1 * groom_give ; Let (100%) of the grooming time go to the most favored bat
...
; Bats then ask for donation if they didn't find food previously
to share
  if fed? = FALSE
  [ ; If the bat didn't find food
    ...
    [ ; And if there are any
      ...
      let sorted-who-list sort flattened-who-list ; And sort it, THIS IS INCORRECT
      let my-bat 0 ; Initialize a local variable for the chosen bat
      let i 0 ; And make an index
      ...
      [ ; While we haven't looked at every bat
        ...
        [ ; Ash the potential donating bat
          ...
          set eaten fed? ; And mark whether they have eateb or not
        ]
        ifelse b < p_donation and R > 0 and eaten = True
        [ ; If marked to donate, and there are reserves to share
          ...
          while [done_feeding = False or fed? = False] ;or R > 0]
          [ ; While the bat is not done feeding or
...
"pen-2" 1.0 0 -16777216 true "" "plot count turtles with [decision = 1]"
